...
17. **Generated actionlint configuration was unsupported** — the aggregate had manually added `actionlint` commands even though the pinned fragment set has no actions integration or materialized wrapper; removed the unsupported commands and restored generation from the declared fragments.
18. **Actions fragment was not materialized in runtime paths** — the selected fragment set includes `.github/workflows` actionlint checks, but runtime inputs omitted `lefthook-actionlint`; added the repository wrapper to devShell/app paths and regenerated `lefthook.yml`.
